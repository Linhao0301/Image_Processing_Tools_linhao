...
class ImageAlphaBlend:
    ...
    def blend_images(self, base_image, overlay_image):
        # 确保输入是numpy数组
        ...
        for i in range(b):
            ...
            if overlay[i].shape[-1] == 4:
                alpha = overlay[i, :, :, 3:4]
                rgb = overlay[i, :, :, :3]
                result[i, :, :, :3] = result[i, :, :, :3] * (1 - alpha) + rgb * alpha
...
NODE_CLASS_MAPPINGS = {
    "ImageAlphaBlend": ImageAlphaBlend
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "ImageAlphaBlend": "Image Alpha Blend(linhao)"
} 